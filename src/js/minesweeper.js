...
export function minesweeper() {
    ...
    if (canvasCache) {
        ...
        if (window.innerWidth > window.innerHeight) {
            scale = Math.floor(window.innerWidth/690);
            canvas.width = 690*scale;
            canvas.height = 390*scale;
        } else {
            scale = Math.floor(window.innerHeight/690);
    ...
    let scale5 = 5*scale;
    let scale12 = 12*scale;
    let scale20 = 20*scale;
    let scale25 = 25*scale;
    let scale40 = 40*scale;
    ...
    function render() {
        ...
        ctx.fillRect(scale20, scale20, canvas.width - scale40, canvas.height - scale40);
        ctx.fillStyle = 'green';
        ctx.font = 'bold 12px Arial';
        ctx.fillText(`🚩: ${mineCountCopy - mineCount}`, (canvas.width - ctx.measureText(`🚩: ${mineCountCopy - mineCount}`).width) / 2, 13)
        for (let i = 0; i <= cols - 2; i++) {
            ctx.beginPath();
            ctx.moveTo(scale20 + ((canvas.width - scale40) * i / (cols - 2)), ((canvas.height - scale40) / (rows - 2)) - scale5);
            ctx.lineTo(scale20 + ((canvas.width - scale40) * i / (cols - 2)), canvas.height - scale20);
            ctx.stroke();
        }

        for (let j = 0; j <= rows - 2; j++) {
            ctx.beginPath();
            ctx.moveTo(((canvas.width - scale40) / (cols - 2)) - scale5, scale20 + ((canvas.height - scale40) * j / (rows - 2)));
            ctx.lineTo(canvas.width - scale20, scale20 + ((canvas.height - scale40) * j / (rows - 2)));
            ctx.stroke();
        }
        for (let i = 1; i < cols - 1; i++) {
            for (let j = 1; j < rows - 1; j++) {
                if (mineField[i][j] !== 10 && mineField[i][j] !== 9) {
                    if (mineField[i][j] > 50) {
                        ctx.fillStyle = 'red';
                        ctx.font = `${scale20}px Arial`;
                        ctx.fillText("🚩", -8*scale + ((canvas.width - scale40) * i / (cols - 2)), 14*scale + ((canvas.height - scale40) * j / (rows - 2)));
                    }
                    if (mineField[i][j] === 0) {
                        ctx.fillStyle = 'darkgray';
                    }
                    ...
                    if (mineField[i][j] < 50) {
                        ctx.font = `bold ${scale12}px Arial`;
                        ctx.fillText(mineField[i][j] + "", 4*scale + ((canvas.width - scale40) * i / (cols - 2)), 12*scale + ((canvas.height - scale40) * j / (rows - 2)));
                    }
                    if (mineField[i][j] === -1) {
                        ctx.font = `${scale20}px Arial`;
                        ctx.fillStyle = 'red'
                        ctx.fillText("💣", -8*scale + ((canvas.width - scale40) * i / (cols - 2)), 16*scale  + ((canvas.height - scale40) * j / (rows - 2)));
                    }
                }
            }
        }
    ...
    function renderControls() {
        ...
        ctx.font = `${scale20}px Arial`;
        ctx.fillStyle = 'black';
        ctx.fillText(playPauseText, (canvas.width - ctx.measureText(playPauseText).width) / 2, canvas.height / 2 - 30*scale)
        ctx.fillText(controlsText, (canvas.width - ctx.measureText(controlsText).width) / 2, canvas.height / 2)
        ctx.fillText(flagText, (canvas.width - ctx.measureText(flagText).width) / 2, canvas.height / 2 + 30*scale)
        ctx.fillText(flagText2, (canvas.width - ctx.measureText(flagText2).width) / 2, canvas.height / 2 + 50*scale)
    }
    ...
    function mouseClicked(e) {
        const normalisedCoordinates = [Math.ceil((mouseCoordinates[0] - scale20) / scale25), Math.ceil((mouseCoordinates[1] - scale20) / scale25)]
        console.log(normalisedCoordinates)
        if (gameOver) {
            gameOver = false
            firstClick = true
            mineCount = 57;
