...
export function canvas_home(user) {// Get a reference to the canvas element
    pushToHome.value = false;
    let renderBeforeStop = 200;
    ...
    let staticCount = 0;
    function setRectangles(user) {
        if (user) {
            rectangles.splice(0, rectangles.length, ...loggedInRectangles);
        } else {
            rectangles.splice(0, rectangles.length, ...loggedOutRectangles);
        }
    ...
    function changePageSize(index, magnitude) {
        ...
        if (magnitude < max_magnitude) {
            if (r.iframe) {
                ...
                try{document.body.removeChild(r.iframe);}catch{}
                r.iframe = null;
            }
        }
